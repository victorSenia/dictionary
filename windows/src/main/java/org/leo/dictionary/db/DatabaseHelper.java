...
public class DatabaseHelper implements DatabaseConstants<Statement> {
    public static final String DB_URL = "jdbc:sqlite:" + DATABASE_NAME;
    ...
    public void ensureInitialized(Connection conn) {
        if (conn != null) {
    ...
    private void setDatabaseVersion(Connection conn) {
        try (Statement statement = conn.createStatement()) {
